''' Rate [500/4]: Random -> Target A -> Impacter -> Sleep '''
...
# from CybORG.Agents.SimpleAgents.FiniteStateRedAgent import _choose_host_and_action
# from CybORG.Agents.SimpleAgents.FiniteStateRedAgent import _choose_host
...
class FSMultipleSwitchAgent1(FiniteStateRedAgent):
    ...
    def __init__(self, name=None, np_random=None, agent_subnets=None):
        ...
        self.zone_a_keywords_Impacter = ['operational_zone_b', 'operational_zone_a']
        self.zone_a_keywords = ['restricted_zone_a', 'operational_zone_a']
    ...
    def _is_zone_a_Impacter(self, hostname: str) -> bool:
        if hostname is None:
            return False
        lower = hostname.lower()
        return any(k in lower for k in self.zone_a_keywords_Impacter)
        
    def _is_zone_a(self, hostname: str) -> bool:
        if hostname is None:
            return False
        lower = hostname.lower()
        return any(k in lower for k in self.zone_a_keywords)
        
    def get_action(self, observation: dict, action_space):
        if (self.step % (self.freq*4)) < self.freq: #Rand
            action = None
            success = None
        ...
        elif (self.step % (self.freq*4)) < 2*self.freq: #TargetA
            action = None
            success = None
            if 'success' in observation.keys():
                success = observation.pop('success')
            ...
            if success.name == 'IN_PROGRESS':
                ...
                chosen_host, action = self._choose_host_and_action_TargetA(action_space, known_hosts)
                if isinstance(action, ExploitRemoteService) and chosen_host in list(self.host_service_decoy_status.keys()):
                    action.exploit_action_selector = PIDSelectiveExploitActionSelector(excluded_pids=self.host_service_decoy_status[chosen_host])
                self.step += 1
                self.last_action = action
        ...
        elif (self.step % (self.freq*4)) < 3*self.freq: #Impacter
            action = None
            success = None
            ...
            if success.name == 'IN_PROGRESS':
                ...
                known_hosts = [h for h in self.host_states.keys() if not self.host_states[h]['state'] == 'F']
                self.host_state_sequence.append(copy.deepcopy(self.host_states))
                chosen_host, action = self._choose_host_and_action_Impacter(action_space, known_hosts)
                if isinstance(action, ExploitRemoteService) and chosen_host in list(self.host_service_decoy_status.keys()):
                    action.exploit_action_selector = PIDSelectiveExploitActionSelector(excluded_pids=self.host_service_decoy_status[chosen_host])
                self.step += 1
                self.last_action = action
    ...
    def _choose_host_Impacter(self, host_options: List[str]):
        ...
        if self.host_states_priority_list is None:
            ...
            for h_opt in host_options:
                ...
                if self._is_zone_a_Impacter(hostname):
                    bool = True
            ...
            for h_opt in host_options:
                ...
                # --- Zone B boost ---
                hostname = self.host_states[h_opt].get('hostname')
                if self._is_zone_a_Impacter(hostname): #and state not in ('RD'):
    ...
    def _choose_host_and_action_Impacter(self, action_space: dict, host_options: List[str]):
        """The selection of a valid host and action to execute this step."""
        chosen_host = self._choose_host_Impacter(host_options)
        if chosen_host == None:
            return Sleep()
        ...
        while True:
            ...
            if len(options) > 0:
                ...
                is_zone_a = self._is_zone_a_Impacter(hostname)
                probabilities = []
                for opt in options:
                    name = opt.__name__.lower()
                    if is_zone_a:
                        # --- Zone A → exploitation focus ---
                        if "impact" in name:
                            host_action_options[opt] *= 4
    
                    elif hostname is None:
                        # --- Non-Zone A → discovery focus ---
                        if "deception" in name:
                            host_action_options[opt] *= 2
                    else: 
                        if "impact" in name:
                            host_action_options[opt] = 0
                        if "deception" in name:
                            host_action_options[opt] *= 2
    ...
    def _choose_host_TargetA(self, host_options: List[str]):
        """A valid host is selected and returned"""
        bool = False
        if self.host_states_priority_list is None:
            # print("is On")
            ...
            for h_opt in host_options:
                ...
                # --- Zone B boost ---
                hostname = self.host_states[h_opt].get('hostname')
                if self._is_zone_a(hostname) and state not in ('RD'):
                    bool = True
                    weight *= 100  # boost Zone B priority
                if hostname is None and state not in ('KD', 'SD'):
                    # print("hosstname None" + state)
    ...
    def _choose_host_and_action_TargetA(self, action_space: dict, host_options: List[str]):
        """The selection of a valid host and action to execute this step."""
        chosen_host = self._choose_host_TargetA(host_options)
        if chosen_host == None:
            return Sleep()
        ...
        while True:
            ...
            if len(options) > 0:
                hostname = self.host_states[chosen_host].get('hostname')
                is_zone_a = self._is_zone_a(hostname)
                probabilities = []
                for opt in options:
                    name = opt.__name__.lower()
                    if is_zone_a:
