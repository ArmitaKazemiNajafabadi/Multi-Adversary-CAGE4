...
class FSRedAgentFailureTracker(BaseAgent):
    """
    GPT:
    1. Deception Failure Tracking:

    Added self.deception_failures dictionary to track failures per host
    Updates count when DiscoverDeception fails in S/SD states
    
    2. Weighted Host Selection:

    Replaced uniform np_random.choice() with weighted selection
    Weight formula: weight = 1.0 / (1.5^failures) with minimum 0.1
    Hosts with more failures get exponentially lower selection probability
    
    3. Enhanced Action Selection:
    
    Added 30% chance to try DiscoverDeception in S/SD states
    Stops attempting after 3 failures per host
    Modified probability matrix for better exploitation focus
    
    4. Key Methods:
    
    _choose_host(): WEIGHTED + Implements weighted selection
    """
    ...
    def _choose_host_and_action(self, action_space: dict, host_options: List[str]):
        ...
        while True:
            ...
            if len(options) > 0:
                probabilities = []
                for opt in options:
                    # state = self.host_states.get(chosen_host, 'K')
                    state = self.host_states.get(chosen_host, {}).get('state', 'K')
                    # print(state)
                    # Special handling for DiscoverDeception in S/SD states
                    if state in ['S', 'SD'] and self.deception_failures.get(chosen_host, 0) >= 3:
                        # Try DiscoverDeception if not failed too many times
