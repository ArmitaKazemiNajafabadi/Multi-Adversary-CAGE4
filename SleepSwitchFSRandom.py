''' Finite State Red Agent that Sleeps with a Frequency of alpha '''
...
class SleepSwitchFSRandom(BaseAgent):
    ...
    def get_action(self, observation: dict, action_space):
        ...
        if (self.step % (self.freq*2)) < self.freq:
            # print("sleeping")
            action = Sleep()
        else:
            action = None
    ...
    def _choose_host(self, host_options: List[str]):
        ...
        # Building a probability distribution over host states (not hosts directly).

        # Sampling a state based on the priority list weights.

        # Narrowing down to only hosts in that state.
...
# }
        ...
        # Does the "else" usually run or the "if" ?? How is better to change this code to make it targetted toward zone B?
