...
class BlueFlatWrapper(BlueFixedActionWrapper):
    ...
    def reset(self, *args, **kwargs) -> tuple[dict[str, Any], dict[str, Any]]:
        ...
        for a in self.agents:
            ...
            ''' added '''
            if self.eval_mode:
                    # In evaluation → return master-level observation only
                    # obs[a] = {
                    #     "observations": self.output[a],
                    #     "action_mask": self.high_actionmask[a],   # [1,0] investigate (default)
                    # }
                    # inf[a] = {}

                    # MASTER policy sees the full obs
                    master = a + "_master"
                    obs[master] = {
                        "observations": self.output[a],
                        "action_mask": self.high_actionmask[a],
                    }
                    inf[master] = {}
                    ...
                    obs[investigate] = {
                        "observations": self.obs_investigate[a],
                        "action_mask": self.investigate_mask[a],
                    }
                    inf[investigate] = {}
                    ...
                    recover = a + "_recover"
                    obs[recover] = {
                        "observations": self.obs_recover[a],
                        "action_mask": self.recover_mask[a],
                    }
                    inf[recover] = {}
        ...
        for agent, o in sorted(observations.items()):
            ...
            if "blue" in agent:
                ...
                if self.eval_mode:
                        ...
                        # obs[master] = {
                        #     "observations": self.output[agent],
                        #     "action_mask": self.high_actionmask[agent],
                        # }
                        # inf[master] = {}
                    
                        # # INVESTIGATE policy
                        # investigate =  agent + "_investigate"
                        # obs[investigate] = {
                        #     "observations": self.obs_investigate[agent],
                        #     "action_mask": self.investigate_mask[agent],
                        # }
                        # inf[investigate] = {}
                    
                        # # RECOVER policy
                        # recover = agent + "_recover"
                        # obs[recover] = {
                        #     "observations": self.obs_recover[agent],
                        #     "action_mask": self.recover_mask[agent],
                        # }
                        # inf[recover] = {}
                        ...
                        # Only expose the single top-level agent during evaluation
                        base = agent  # example: "blue_agent_0"
                    
                        # The master policy decides internally which subpolicy to run
                        # But externally we only return the master-level observation
                        obs[base] = {
                            "observations": self.output[agent],       # 76-dim obs
                            "action_mask": self.high_actionmask[agent]  # 2-dim mask for master
                        }
                    
                        # Reward/term/trunc must match the base agent
                        rew[base] = rewards[agent]
                        term[base] = terminated[agent]
                        trunc[base] = truncated[agent]
                    
                        # Info can be empty or return the mask
                        inf[base] = {"action_mask": self.high_actionmask[agent]}
                    
                        # update previous (if used)
