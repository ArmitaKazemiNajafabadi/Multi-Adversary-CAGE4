...
""" the global_state_space=3 has changed to global_state_space=15 since phase also has attacker_id"""
...
class SimpleSelfAttention(nn.Module):
    ...
    def forward(self, v, mask, g=None):
        ...
        Inputs:
            ...
            g:      B x d tensor
        '''
        # print(f"[DEBUG] att input shape: {v.shape}, g.shape: {g.shape if g is not None else None}")
...
class InductiveActorNetwork(nn.Module):
    def __init__(self, in_dim, global_state_space=15,
                 node_action_space=4, edge_action_space=2, global_action_space=1,
                 hidden1=320, hidden2=64, gdim=64, lr=0.0003, concat_edges=False):
    ...
    def forward(self, x, ei, global_vec, servers, n_servers, users, n_users, action_edges, multi_subnet):
        # print(f"[DEBUG] actor forward x.shape: {x.shape}")
        # print(f"[DEBUG] actor forward global_vec.shape: {global_vec.shape}")
...
class InductiveCriticNetwork(nn.Module):
    def __init__(self, in_dim, global_state_space=15,
                 hidden1=320, hidden2=64, gdim=64, lr=0.001):
        super().__init__()
