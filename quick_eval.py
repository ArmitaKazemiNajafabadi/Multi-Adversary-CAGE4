...
from wrapper.graph_wrapper_with_classifier_quick import GraphWrapper
...
class Submission:
    ...
    # Submission name
    NAME: str = "Context-Aware Defense"
    TEAM: str = "Us"
    TECHNIQUE: str = "Proposed"
    AGENTS = {
        f"blue_agent_{i}": load(f'{os.path.dirname(__file__)}/results/agent_{i}_checkpoint.pt') #The model trained on FS, Impacter, degrader, failure tracker, combined Red agents.
        for i in range(5) 
    }
