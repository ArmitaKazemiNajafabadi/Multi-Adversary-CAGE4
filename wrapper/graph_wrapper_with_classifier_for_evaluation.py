""" MAKE SURE THE WRAPPER USES THE BELIEF>UPDATE"""
from copy import deepcopy
# from train_with_classifier import BeliefClassifier
# from train_onehot_classifier_2 import BeliefCNNClassifier # Nov 21, 2025 commented
...
class GraphWrapper(EnterpriseMAE):
    def __init__(self, env: CybORG, window_size, total_attaacker_num, *args, **kwargs):
        ...
        # self.belief += (1/(12*12))
        # self.belief[0][1] = (1/(12*12))
        # self.belief = self.belief / self.belief.sum(dim=1, keepdim=True)
        # print(self.belief)
        ...
        self.stacked_obs = []
        '''Nov 21, 2025 switch'''
        # self.classifier = BeliefCNNClassifier(input_dim=763, seq_len=self.L, num_classes=total_attaacker_num)
        # self.classifier.load_state_dict(torch.load(f'/projects/ImaniLab/Armita/CAGE-Multi-Adversary/checkpoints_onehot_att/Oct18_CNNclassifier_{self.L}_checkpoint.pt', map_location='cpu'))
    ...
    """ update_belief Pre 21 Nov 2025 Version """
    # def update_belief(self):
    #     if len(self.stacked_obs) >= self.L:
    #         suitable_input = torch.cat(self.stacked_obs, dim=0) # L rows each with 763-len features
    #         # print(suitable_input.size())
    #         with torch.no_grad():
    #             # print(f"size input {suitable_input.size()}")
    #             model_out = self.classifier(suitable_input.unsqueeze(0)) 
    #             probs = F.softmax(model_out, dim=-1)
    #             # print(probs.size())
    #         self.belief = probs.clone()
            # print(f'argmax new belief: {torch.argmax(self.belief)} - true attack prob in belief: {self.belief[0][0]} - new belief itself:{(self.belief)} ' )
    ...
    def update_belief(self, b_lr=0.05):
        ...
        MAX_SEQ = 10 if self.ts > 10 else self.ts  # Added Apr 30 , 2026 as a result of revision
    ...
    def get_belief(self):
        return self.belief.detach().cpu().numpy().tolist()
    ...
    def step(self, action):
        ...
        ''' Pre 21 Nov 2025 Version '''
        # concatenation = torch.cat(states_for_classifier, dim=1)
        # if len(self.stacked_obs) >= self.L:
        #     self.stacked_obs.pop(0)
        # self.stacked_obs.append(concatenation)
        # self.update_belief(ts)
        ''' 21 Nov 2025 Version '''
        concatenation = torch.cat(states_for_classifier, dim=1)
    ...
    def _to_obs(self, x,ei,masks,phase, other_msg,my_msg):
        ...
        if len(masks) == 1:
            ...
            '''added part'''
            # print(f"[DEBUG] x shape before attacker injection: {x.shape}")
            # Add attacker one-hot to each node feature
            ...
            # print(f"[DEBUG] x shape after attacker injection: {x.shape}")
            # print(f"[DEBUG] attacker_id {self.attacker_id}, attacker slice (first 3 rows):\n{x[:3, -self.total_attaacker_num:]}")
            '''end added part'''
            return (
                x,ei,phase,
        ...
        # Repeat for all nodes or add as global feature
